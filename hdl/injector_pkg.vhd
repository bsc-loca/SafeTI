...
package injector_pkg is
  ...
  constant MAX_STATUS_LEN   : integer := 8;   -- Maximum length of the internal status flag field from any stage.
  constant EXE_N_SUBMODULES : integer := 4;   -- Number of task submodules on the EXE stage.
  ...
  constant OP_OBSERVER_ADDR : std_logic_vector(4 downto 0) := "10000";
  constant OP_OBSERVER_HOLD : std_logic_vector(4 downto 0) := "10001";
  constant OP_META          : std_logic_vector(4 downto 0) := "11111"; -- TODO: Use META type to add new descriptor words.

  -- Debug states
  subtype DEBUG_STATE is std_logic_vector(MAX_STATUS_LEN - 1 downto 0);
  constant DEBUG_STATE_IDLE               : DEBUG_STATE := x"00"; -- Common debug states
  constant DEBUG_STATE_PROGRAMMED         : DEBUG_STATE := x"01"; -- EXE debug states
  constant DEBUG_STATE_OPERATION          : DEBUG_STATE := x"02";
  constant DEBUG_STATE_DATA_TRANSFER      : DEBUG_STATE := x"03";
  constant DEBUG_STATE_LAST               : DEBUG_STATE := x"06";
  constant DEBUG_STATE_LAST_TRANSFER      : DEBUG_STATE := x"07";
  constant DEBUG_STATE_HOLD               : DEBUG_STATE := x"08";
  constant DEBUG_STATE_1st_DECODE         : DEBUG_STATE := x"09"; -- DECODE debug states
  constant DEBUG_STATE_REPETITION         : DEBUG_STATE := x"0A";
  -- Error debug states
  constant DEBUG_STATE_UNEXPECTED_DATA    : DEBUG_STATE := x"10"; -- READ and WRITE error debug states
  constant DEBUG_STATE_UNEXPECTED_DONE    : DEBUG_STATE := x"11";
  constant DEBUG_STATE_UNEXPECTED_START   : DEBUG_STATE := x"18"; -- Common error states

  -- Submodule IDs
  constant SUBM_ID_DELAY    : integer := 0;
  constant SUBM_ID_READ     : integer := 1;
  constant SUBM_ID_WRITE    : integer := 2;
  constant SUBM_ID_OBSERVER : integer := 3;

  -- CSR register offsets
  constant CSR_OFFSET_CONFIG        : std_logic_vector(7 downto 0) := x"00"; -- R/W
  constant CSR_OFFSET_DEBUG_PC      : std_logic_vector(7 downto 0) := x"60"; -- R
  constant CSR_OFFSET_CNT_INT       : std_logic_vector(7 downto 0) := x"80"; -- R/W
  constant CSR_OFFSET_CNT_ACCESS    : std_logic_vector(7 downto 0) := x"84"; -- R/W
  constant CSR_WRITE_DATA           : std_logic_vector(7 downto 0) := x"F0"; -- R/W
  constant CSR_OFFSET_NET_PROFILE   : std_logic_vector(7 downto 0) := x"F8"; --   W
  constant CSR_OFFSET_DESC_W_INPUT  : std_logic_vector(7 downto 0) := x"FC"; --   W
  ...
  type ib_miso is record  --Input to injector_ctrl from bus master interface output
    ...
    wr_done           : std_logic;
    wr_err            : std_logic;
    -- External channel (observer)
    external_addr     : std_logic_vector(31 downto 0);
  end record ib_miso;
  ...
  type csr_out is record -- Lacks config, but it is added on the platform wrapper.
  ...
  type injector_config is record
    ...
    hold              : std_logic;  -- Injector prepare and hold operations
  ...
  -- Decoded descriptor type into a bit array of the submodules to control the operation.
  subtype submodule_bit is std_logic_vector(0 to EXE_N_SUBMODULES-1);

  -- Decoded descriptor for READ and WRITE descriptor types
  type operation_rd_wr is record
    size              : std_logic_vector(18 downto 0);  -- Encoded burst transfer size of the next transaction.
  ...
  type operation_delay is record
    num_cycles        : std_logic_vector(18 downto 0);
  end record operation_delay;

  -- Decoded descriptor for OBSERVER_ADDR and OBSERVER_HOLD descriptor type
  type operation_observer is record
    set_addr          : std_logic;                      -- Program observer with target address to look for.
    set_mask          : std_logic_vector(18 downto 0);  -- Programable LSB mask for address to hold for.
    set_hold          : std_logic;                      -- Next descriptor executed when triggered by observer.
  end record operation_observer;
  ...
  type bus_decode_exe is record
    ...
    observer          : operation_observer;     -- Control data to execute OBSERVER_ADDR and OBSERVER_HOLD operations
  ...
  -- Submodule bit array
  constant RESET_SUBMODULE_BIT : submodule_bit := (submodule_bit'range => '0');

  -- READ and WRITE descriptor (two 32-bit words)
  ...
  component injector_core is
    generic (
      ...
      PC_LEN            : integer range 2 to   14       :=    4;  -- Set the maximum number of programmable descriptor words to 2^PC_LEN
